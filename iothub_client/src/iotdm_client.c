...
static char* GetDeviceFromTopic(const char* topic, SHADOW_CALLBACK_TYPE* type)
{
    ...
    else if (StringCmp(TOPIC_SUFFIX_METHOD_CLOUD_RESP, topic, tmp = (strrchr(topic, '/') - topic) - strlen(TOPIC_SUFFIX_METHOD_CLOUD_RESP), (strrchr(topic, '/') - topic))) {
        *type = SHADOW_CALLBACK_TYPE_METHOD_RESP;
        end = tmp - 1;
    }
    else
...
int iotdm_client_dowork(const IOTDM_CLIENT_HANDLE handle)
{
    ...
    {
//        LogInfo("Subscribing"); // zhaobo03 debug logging
        size_t topicSize = CalTopicSize(handle);
        char* topics[topicSize];
        int amount = GetSubscription(handle, topics, topicSize);
        if (amount < 0)
    ...
    if (handle->mqttClient->isConnectionLost && handle->mqttClient->mqttClientStatus == MQTT_CLIENT_STATUS_CONNECTED
            && handle->enableOta)
    {
        // TODO: this can be removed in future if cloud automatically detect if device has finished subscribing.
        char* requestId = GenRequestId();
        int result = iotdm_client_ota_get_job(handle, NULL, requestId);
        if (result != 0)
        {
            LogError("Failed to get OTA job");
        }
    }
...
int iotdm_client_method_req(const IOTDM_CLIENT_HANDLE handle, const char* device, const char* methodName, JSON_Value* payload, const char* requestId) {
    if (NULL == requestId)
    {
        LogError("Failure: request id should not be NULL.");
        if (payload != NULL) {
                                     ...
                                     const char *requestId)
{
    ...
    return iotdm_client_method_req(handle, handle->name, METHOD_GET_FIRMWARE, request, requestId);
}
...
int iotdm_client_ota_report_start(const IOTDM_CLIENT_HANDLE handle, const char* jobId, const char* requestId)
{
    ...
    return iotdm_client_method_req(handle, handle->name, METHOD_REPORT_FIRMWARE_UPDATE_START, request, requestId);
}
...
int iotdm_client_ota_report_result(const IOTDM_CLIENT_HANDLE handle, const char* jobId, bool isSuccess, const char* requestId)
{
    ...
    json_object_set_string(root, KEY_JOB_ID, jobId);
    return iotdm_client_method_req(handle, handle->name, METHOD_REPORT_FIRMWARE_UPDATE_RESULT, request, requestId);
}
